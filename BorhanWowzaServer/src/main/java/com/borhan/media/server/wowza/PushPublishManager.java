...
import com.borhan.client.types.BorhanLiveStreamPushPublishConfiguration;
import com.borhan.client.types.BorhanLiveStreamPushPublishRTMPConfiguration;
...
import com.borhan.media.server.managers.BorhanLiveManager;
import com.borhan.media.server.managers.BorhanManager;
...
public class PushPublishManager extends BorhanManager implements IBorhanEventConsumer, ILiveManager.ILiveEntryReferrer {
	...
	public void init() throws BorhanManagerException {
		...
		BorhanEventsManager.registerEventConsumer(this, BorhanEventType.STREAM_UNPUBLISHED, BorhanEventType.STREAM_PUBLISHED, BorhanMediaEventType.MEDIA_STREAM_PUBLISHED);
		liveManager = (ILiveManager) BorhanServer.getManager(ILiveManager.class);
	...
	public void onEvent(IBorhanEvent event) {
		...
		if(event.getType() instanceof BorhanMediaEventType){
			BorhanMediaStreamEvent mediaStreamEvent;
			switch((BorhanMediaEventType) event.getType())
			{
				case MEDIA_STREAM_PUBLISHED:
					mediaStreamEvent = (BorhanMediaStreamEvent) event;
		...
		if(event.getType() instanceof BorhanEventType){
			switch((BorhanEventType) event.getType())
			{
				case STREAM_PUBLISHED:
					BorhanMediaStreamEvent publishStreamEvent = (BorhanMediaStreamEvent) event;
				...
				case STREAM_UNPUBLISHED:
					BorhanStreamEvent streamEvent = (BorhanStreamEvent) event;
	...
	protected void onMediaStreamPublish (IMediaStream stream, BorhanLiveEntry entry, int assetParamsId)
	{
		if (entry.pushPublishEnabled == BorhanLivePublishStatus.DISABLED)
		{
			return;
		}
		...
		if (!streamName.contains("@") && !streamName.startsWith("push-")) {
			...
			BorhanLiveAsset liveAsset = liveManager.getLiveAsset(entry.id, assetParamsId);
			if (liveAsset == null) {
	...
	protected void onSourcePublish (IMediaStream stream, BorhanLiveEntry entry, int assetParamsId, BorhanMediaServerIndex serverIndex)
	{
		if (entry.pushPublishEnabled == BorhanLivePublishStatus.DISABLED)
		{
			return;
	...
	protected void multicastPublish (IMediaStream stream, BorhanLiveEntry entry, BorhanLiveAsset liveAsset)
	{
		String streamName = stream.getName();
...
//		    //Update the liveAsset
	    ...
	    try
	    {
	    	BorhanClient impersonateClient = impersonate(entry.partnerId);
	    	impersonateClient.startMultiRequest();
	    	impersonateClient.getFlavorAssetService().update(liveAsset.id, liveAsset);
	    	impersonateClient.getLiveStreamService().addLiveStreamPushPublishConfiguration(entry.id, BorhanPlaybackProtocol.MULTICAST_SL, publisher.getHostname() +  ":" + publisher.getPort());
	    	BorhanMultiResponse responses = impersonateClient.doMultiRequest();
	    	
	    	for (Object response : responses) {
	    		if (response instanceof BorhanApiException) {
	    			logger.error("Error occured during request: [" +  ((BorhanApiException)response).getMessage() + "]");
	    		}
			}
	    }
	...
	protected void rtmpPublish (IMediaStream stream, BorhanLiveEntry entry, int assetParamsId, BorhanMediaServerIndex serverIndex) {
		try {
			ArrayList<BorhanLiveStreamPushPublishConfiguration> configurations = entry.publishConfigurations;
			for (BorhanLiveStreamPushPublishConfiguration configuration : configurations) { 
				if (configuration instanceof BorhanLiveStreamPushPublishRTMPConfiguration) {
					...
					BorhanLiveStreamPushPublishRTMPConfiguration rtmpConfiguration = (BorhanLiveStreamPushPublishRTMPConfiguration)configuration;
					...
					if (BorhanMediaServerIndex.PRIMARY.equals(serverIndex) ) {
						publisher.setHost(rtmpConfiguration.publishUrl);
					}
					else if (BorhanMediaServerIndex.SECONDARY.equals(serverIndex)) {
	...
	protected void onUnPublish (BorhanLiveEntry entry) {
		try {				
			...
			try {
				BorhanClient impersonateClient = impersonate(entry.partnerId);
				impersonateClient.getLiveStreamService().removeLiveStreamPushPublishConfiguration(entry.id, BorhanPlaybackProtocol.MULTICAST_SL);
			}
			catch (BorhanApiException e) {
				logger.error("Operation failed. Exception message:  [" + e.getMessage() + "]");
			}
