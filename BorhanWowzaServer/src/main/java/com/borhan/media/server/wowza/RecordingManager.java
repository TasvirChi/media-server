...
public class RecordingManager {
	...
	private BorhanLiveManager liveManager;
	...
	class EntryRecorder extends LiveStreamRecorderMP4 implements ILiveStreamRecordNotify, ILiveManager.ILiveEntryReferrer
	{
		...
		private BorhanMediaServerIndex index;
		...
		public EntryRecorder(String entryId, String assetId, BorhanMediaServerIndex index) {
			super();
		...
		public void onSegmentEnd(ILiveStreamRecord liveStreamRecord) {
			...
			AppendRecordingTimerTask appendRecording = new AppendRecordingTimerTask(file.getAbsolutePath(), isLastChunk, (double)this.getCurrentDuration()/1000) {
				...
				public void run() {
					...
					Map<String, Object> serverConfiguration = BorhanServer.getConfiguration();
		...
		public void onUnPublish () {
			...
			BorhanLiveAsset liveAsset = liveManager.getLiveAssetById(entryId, assetId);
			logger.info("current media server index: " + index);
			if (liveAsset != null && liveAsset.tags.contains(RecordingManager.RECORDING_ANCHOR_TAG_VALUE) && BorhanMediaServerIndex.PRIMARY.equals(index)) {
	...
	public RecordingManager(BorhanLiveManager liveManager) throws BorhanManagerException {
		...
		if(!BorhanServer.isWindows()){
			synchronized (groupInitialized) {
				if(!groupInitialized){
					...
					Map<String, Object> serverConfiguration = BorhanServer.getConfiguration();
					if (serverConfiguration.containsKey(RecordingManager.BORHAN_RECORDED_FILE_GROUP))
						groupName = (String) serverConfiguration.get(RecordingManager.BORHAN_RECORDED_FILE_GROUP);
		...
		synchronized (recorders)
		{
			...
			if (entryRecorders != null){
				for(EntryRecorder streamRecorder : entryRecorders.values()){
					BorhanLiveAsset liveAsset = liveManager.getLiveAssetById(entryId, streamRecorder.getAssetId());
	...
	public String start(String entryId, String assetId, IMediaStream stream, BorhanMediaServerIndex index, boolean versionFile, boolean startOnKeyFrame, boolean recordData){
