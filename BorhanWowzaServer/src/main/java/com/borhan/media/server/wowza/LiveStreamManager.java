...
import com.borhan.media.server.managers.BorhanLiveStreamManager;
...
public class LiveStreamManager extends BorhanLiveStreamManager {
	...
	protected final static String BORHAN_ASSET_TAG_SOURCE = "source";
	...
	public void init() throws BorhanManagerException {
		...
		BorhanEventsManager.registerEventConsumer(this, 
				BorhanMediaEventType.MEDIA_STREAM_PUBLISHED,
				BorhanEventType.STREAM_PUBLISHED,
				BorhanEventType.STREAM_UNPUBLISHED,
				BorhanEventType.STREAM_READY_FOR_PLAYBACK);
		setInitialized();
	}
		...
		if (event.getType() instanceof BorhanEventType) {
			switch((BorhanEventType) event.getType())
			{
				...
				case STREAM_UNPUBLISHED:
					BorhanMediaStreamEvent streamUnpublishedEvent = (BorhanMediaStreamEvent) event;
	...
	protected void onMediaStreamPublish(IMediaStream stream, String entryId, BorhanMediaServerIndex serverIndex, int assetParamsId) {
		...
		if(liveEntry.recordStatus == null || liveEntry.recordStatus == BorhanRecordStatus.DISABLED){
			logger.info("Entry [" + entryId + "] recording disabled");
			return;
		}
		
		BorhanLiveAsset liveAsset = getLiveAsset(entryId, assetParamsId);
		if(liveAsset == null){
