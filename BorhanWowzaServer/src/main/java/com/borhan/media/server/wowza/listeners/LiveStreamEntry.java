...
import com.borhan.media.server.events.BorhanMetadataEvent;
import com.borhan.media.server.events.BorhanStreamEvent;
import com.borhan.media.server.managers.ILiveManager;
import com.borhan.media.server.managers.ILiveStreamManager;
import com.borhan.media.server.wowza.LiveStreamManager;
import com.borhan.media.server.wowza.SmilManager;
import com.borhan.media.server.wowza.ZombieStreamsWatcher;
import com.borhan.media.server.wowza.events.BorhanApplicationInstanceEvent;
import com.borhan.media.server.wowza.events.BorhanMediaEventType;
import com.borhan.media.server.wowza.events.BorhanMediaStreamEvent;
...
public class LiveStreamEntry extends ModuleBase {
	...
	private class DvrRecorderControl implements ILiveStreamDvrRecorderControl, ILiveStreamPacketizerControl {
		...
		private boolean isThatStreamNeeded(IMediaStream stream) {
			...
			BorhanLiveStreamEntry liveStreamEntry = liveStreamManager.get(entryId);
			...
			if (liveStreamEntry.dvrStatus != BorhanDVRStatus.ENABLED) {
	...
	class LiveStreamListener extends MediaStreamActionNotifyBase implements ILiveManager.ILiveEntryReferrer {
		...
		public void onPublish(IMediaStream stream, String streamName, boolean isRecord, boolean isAppend) {
            ...
            String entryId;
            BorhanMediaServerIndex serverIndex;
			...
			if (properties != null) { // streamed from the client
				...
				serverIndex = BorhanMediaServerIndex.get(properties.getPropertyInt(LiveStreamEntry.CLIENT_PROPERTY_SERVER_INDEX, LiveStreamEntry.INVALID_SERVER_INDEX));

				ILiveStreamManager liveManager = BorhanServer.getManager(ILiveStreamManager.class);
				liveManager.addReferrer(entryId, this);

				BorhanLiveEntry entry = liveStreamManager.get(entryId);
				if (entry == null) {
				...
				if (!entryId.equals(streamName)) {
					...
					BorhanLiveAsset liveAsset = liveStreamManager.getLiveAsset(entry.id, streamSuffix);
					if (liveAsset != null) {
						assetParamsId = liveAsset.flavorParamsId;
					}
				}

				BorhanMediaStreamEvent event = new BorhanMediaStreamEvent(BorhanEventType.STREAM_PUBLISHED, entry, serverIndex, applicationName, stream, assetParamsId);
				BorhanEventsManager.raiseEvent(event);
				...
				BorhanMediaStreamEvent event = new BorhanMediaStreamEvent(BorhanMediaEventType.MEDIA_STREAM_PUBLISHED, liveStreamManager.get(entryId), serverIndex, applicationName, stream, assetParamsId);
				BorhanEventsManager.raiseEvent(event);
			}
		...
		public void onUnPublish(IMediaStream stream, String streamName, boolean isRecord, boolean isAppend) {
			if (stream.isTranscodeResult()) {
				...
				ILiveStreamManager liveManager = BorhanServer
			...
			BorhanLiveStreamEntry liveStreamEntry = liveStreamManager.get(clientProperties.getPropertyStr(LiveStreamEntry.CLIENT_PROPERTY_ENTRY_ID));
			BorhanMediaServerIndex serverIndex = BorhanMediaServerIndex.get(clientProperties.getPropertyInt(LiveStreamEntry.CLIENT_PROPERTY_SERVER_INDEX, LiveStreamEntry.INVALID_SERVER_INDEX));

			logger.debug("UnPublish: " + liveStreamEntry.id);

			BorhanMediaStreamEvent event = new BorhanMediaStreamEvent(BorhanEventType.STREAM_UNPUBLISHED, liveStreamEntry, serverIndex, applicationName, stream);
			BorhanEventsManager.raiseEvent(event);
		...
		public void onRegisterStreamNameGroup(LiveStreamTranscoder liveStreamTranscoder, final TranscoderStreamNameGroup streamNameGroup) {
			...
			TimerTask asyncTask = new TimerTask() {
				...
				public void run() {
					...
					if(tag.equals("all") && entry.dvrStatus == BorhanDVRStatus.ENABLED){
	...
	public void onDisconnect(IClient client) {
		...
		if (clientProperties.containsKey(LiveStreamEntry.CLIENT_PROPERTY_ENTRY_ID)) {
			...
			BorhanStreamEvent event = new BorhanStreamEvent(BorhanEventType.STREAM_DISCONNECTED, liveStreamManager.get(entryId));
			BorhanEventsManager.raiseEvent(event);
		}
	}
	...
	private BorhanLiveEntry onClientConnect(IMediaStream stream) {
		if (stream.getClient() != null) {
	...
	private BorhanLiveEntry onClientConnect(RTPSession rtpSession) {
		String queryString = rtpSession.getQueryStr();

		try {
			return onClientConnect(rtpSession.getProperties(), queryString);
		} catch (BorhanApiException | ClientConnectException e) {
	...
	private BorhanLiveEntry onClientConnect(IClient client) {
		WMSProperties properties = client.getProperties();
	...
	private BorhanLiveEntry onClientConnect(WMSProperties properties, String queryString) throws BorhanApiException, ClientConnectException {
		...
		BorhanLiveEntry entry = liveStreamManager.authenticate(entryId, partnerId, token);
	...
	public boolean setLiveStreamManager() {
		...
		ILiveStreamManager serverLiveStreamManager = BorhanServer.getManager(ILiveStreamManager.class);
	...
	public void onAppStart(IApplicationInstance appInstance) {
		...
		if (properties.containsKey(LiveStreamEntry.APPLICATION_MANAGERS_PROPERTY_NAME)) {
			...
			try {
				if (managers.length()>0) {
					BorhanServer.getInstance().initApplicationManagers(managers.replaceAll(" ", "").split(","));
				}
			} catch (Exception e) {
		...
		BorhanApplicationInstanceEvent event = new BorhanApplicationInstanceEvent(BorhanMediaEventType.APPLICATION_INSTANCE_STARTED, appInstance);
		BorhanEventsManager.raiseEvent(event);
	}
			...
			TimerTask raiseReadyForPlaybackEvent = new TimerTask() {
				...
				public void run() {
					try {
						...
						BorhanMediaStreamEvent event = new BorhanMediaStreamEvent(BorhanEventType.STREAM_READY_FOR_PLAYBACK, liveStreamManager.get(entryId), serverIndex, applicationName, mediaStream);
						BorhanEventsManager.raiseEvent(event);
